...
from core.kelly_sizer import KellyPositionSizer
...
class SimulationRunner:
    def __init__(self, cfg: SimConfig):
        ...
        self.broker = MockBroker(starting_cash=self.portfolio.cash, event_handler=self.events)
        self.sizer = DynamicPositionSizer2(risk_percentage=0.15, max_trade_pct=0.05, max_holding_pct=0.10)
    ...
    async def _on_bar(self, bar: dict) -> None:
        ...
        can_enter_long  = (bar_closed and self.trade_gate.can_enter(symbol, "long", ts, bar_id, regime, allow_pyramiding=False)[0])
        can_enter_short = (bar_closed and self.trade_gate.can_enter(symbol, "short", ts, bar_id, regime, allow_pyramiding=False)[0])
        can_pyr_long    = (bar_closed and self.trade_gate.can_enter(symbol, "long", ts, bar_id, regime, allow_pyramiding=True)[0])
        can_pyr_short   = (bar_closed and self.trade_gate.can_enter(symbol, "short", ts, bar_id, regime, allow_pyramiding=True)[0])
    ...
    async def _bar_producer(self) -> None:
        ...
        for step in range(self.cfg.steps):
            ...
            for bar in all_bars.values():
                canonical = to_canonical_bar(bar)
                payload: BarPayload = {
                    "symbol": canonical["symbol"],
                    "timestamp": canonical["timestamp"].isoformat(),  # if crossing bus
                    "open": canonical["Open"],
                    "high": canonical["High"],
                    "low":  canonical["Low"],
