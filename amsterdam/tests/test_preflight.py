...
class TestEnvironmentChecks:
    ...
    def test_check_environment_missing_alpaca(self, checker):
        ...
        with patch.dict(
            os.environ, {"SCHWAB_API_KEY": "test_schwab_key", "SCHWAB_SECRET": "test_schwab_secret"}, clear=True
        ):
            with patch.object(checker, "print_header"):
                with patch.object(checker, "print_status"):
                    checker._check_environment()
        ...
        with patch.object(checker.validator, "validate_all", new_callable=AsyncMock) as mock_validate:
            mock_validate.return_value = {
                "alpaca": ValidationResult(
                    broker="alpaca", status=CredentialStatus.VALID, message="Valid", can_trade=True, can_fetch_data=True
                ),
                "schwab": ValidationResult(
                    broker="schwab", status=CredentialStatus.VALID, message="Valid", can_trade=True, can_fetch_data=True
                ),
            }
        ...
        with patch.object(checker.validator, "validate_all", new_callable=AsyncMock) as mock_validate:
            mock_validate.return_value = {
                ...
                "schwab": ValidationResult(
                    ...
                    message="Token expiring in 12 hours",
                    can_trade=True,
                    can_fetch_data=True,
                ),
            }
...
class TestDataFreshnessChecks:
    ...
    async def test_check_data_freshness_needs_reprocessing(self, checker):
        ...
        with patch("preflight.UnifiedDataPipeline") as MockPipeline:
            ...
            mock_pipeline._get_last_processed_timestamp.return_value = (
                int(datetime.now(timezone.utc).timestamp() * 1000) - 86400000 * 2
            )
            ...
            with patch.object(checker, "print_header"):
                with patch.object(checker, "print_warning"):
                    ...
                    assert any(
                        "needs reprocessing" in warn.lower() or "needs update" in warn.lower()
                        ...
                        with patch.object(checker, "_print_summary", return_value=True):
                            with patch.object(checker, "print_header"):
                                result = await checker.run_all_checks(
                                    symbols=["AAPL"], update_data=False, reauth_schwab=False
                                )
                                assert result is True
