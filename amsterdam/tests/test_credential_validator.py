...
class TestValidationResult:
    ...
    def test_defaults(self):
        ...
        result = ValidationResult(broker="test", status=CredentialStatus.MISSING, message="Test")
...
class TestCredentialValidator:
    ...
    async def test_validate_schwab_missing_token_file(self, validator):
        ...
        with patch.dict(os.environ, {"SCHWAB_API_KEY": "test_key", "SCHWAB_SECRET": "test_secret"}):
        ...
        with patch.object(validator, "validate_schwab", new_callable=AsyncMock) as mock_schwab:
            with patch.object(validator, "validate_alpaca", new_callable=AsyncMock) as mock_alpaca:
                mock_schwab.return_value = ValidationResult(
                    broker="schwab", status=CredentialStatus.VALID, message="OK", can_trade=True, can_fetch_data=True
                )
                mock_alpaca.return_value = ValidationResult(
                    broker="alpaca", status=CredentialStatus.VALID, message="OK", can_trade=True, can_fetch_data=True
                )
    ...
    def test_get_best_data_source_falls_back_to_schwab(self, validator):
        ...
        results = {
            "alpaca": ValidationResult(
                broker="alpaca", status=CredentialStatus.MISSING, message="Missing", can_fetch_data=False
            ),
            "schwab": ValidationResult(
                broker="schwab", status=CredentialStatus.VALID, message="OK", can_fetch_data=True
            ),
    ...
    def test_get_best_data_source_none_available(self, validator):
        ...
        results = {
            ...
            "schwab": ValidationResult(
                broker="schwab", status=CredentialStatus.EXPIRED, message="Expired", can_fetch_data=False
            ),
        }
    ...
    def test_get_best_trading_broker(self, validator):
        ...
        results = {
            "alpaca": ValidationResult(broker="alpaca", status=CredentialStatus.VALID, message="OK", can_trade=True),
            "schwab": ValidationResult(broker="schwab", status=CredentialStatus.VALID, message="OK", can_trade=True),
        }
...
class TestConvenienceFunctions:
    ...
    async def test_check_credentials(self):
        ...
        with patch("core.credential_validator.CredentialValidator") as MockValidator:
            ...
            mock_validator.validate_all = AsyncMock(
                return_value={"alpaca": ValidationResult(broker="alpaca", status=CredentialStatus.VALID, message="OK")}
            )
    ...
    async def test_can_use_schwab(self):
        ...
        with patch("core.credential_validator.CredentialValidator") as MockValidator:
            mock_validator = MagicMock()
            mock_validator.validate_schwab = AsyncMock(
    ...
    async def test_can_use_alpaca(self):
        ...
        with patch("core.credential_validator.CredentialValidator") as MockValidator:
            mock_validator = MagicMock()
            mock_validator.validate_alpaca = AsyncMock(
                return_value=ValidationResult(
                    broker="alpaca", status=CredentialStatus.VALID, message="OK", can_fetch_data=True
                )
            )
