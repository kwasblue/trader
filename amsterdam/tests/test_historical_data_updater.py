...
class TestHistoricalDataUpdaterInit:
    ...
    def test_init_with_credentials(self):
        ...
        with patch("core.historical_data_updater.Logger"):
            updater = HistoricalDataUpdater(api_key="test_key", api_secret="test_secret")
    ...
    def test_init_custom_path(self, tmp_path):
        ...
        with patch("core.historical_data_updater.Logger"):
            updater = HistoricalDataUpdater(api_key="test_key", api_secret="test_secret", data_path=str(tmp_path))
...
class TestDataFetching:
    ...
    def updater(self, tmp_path):
        with patch("core.historical_data_updater.StockHistoricalDataClient"):
            with patch("core.historical_data_updater.Logger"):
                return HistoricalDataUpdater(api_key="test_key", api_secret="test_secret", data_path=str(tmp_path))
...
class TestDataFreshness:
    ...
    def updater(self, tmp_path):
        with patch("core.historical_data_updater.StockHistoricalDataClient"):
            with patch("core.historical_data_updater.Logger"):
                u = HistoricalDataUpdater(api_key="test_key", api_secret="test_secret", data_path=str(tmp_path))
    ...
    def test_freshness_fresh_data(self, updater, tmp_path):
        ...
        data = [{"Date": now_ms, "Open": 150.0, "High": 151.0, "Low": 149.0, "Close": 150.5, "Volume": 1000}]
    ...
    def test_freshness_stale_data(self, updater, tmp_path):
        ...
        data = [{"Date": old_ms, "Open": 150.0, "High": 151.0, "Low": 149.0, "Close": 150.5, "Volume": 1000}]
        file_path = tmp_path / "proc_AAPL_file.json"
        file_path.write_text(json.dumps(data))
...
class TestIncrementalUpdate:
    ...
    async def test_incremental_update(self, updater, tmp_path):
        ...
        with patch("core.historical_data_updater.StockHistoricalDataClient") as MockClient:
            ...
            await updater.update_symbol("AAPL", days=1, force_full=False)
    ...
    async def test_force_full_update(self, updater, tmp_path):
        ...
        with patch("core.historical_data_updater.StockHistoricalDataClient") as MockClient:
            ...
            await updater.update_symbol("AAPL", days=1, force_full=True)
...
class TestMultiSymbolUpdate:
    ...
    async def test_concurrent_updates(self, updater):
        ...
        with patch("core.historical_data_updater.StockHistoricalDataClient") as MockClient:
            ...
            results = await updater.update_symbols(["AAPL", "MSFT", "GOOGL", "TSLA", "NVDA"], days=1, max_concurrent=3)
...
class TestPeriodicUpdates:
    ...
    async def test_run_periodic_single_iteration(self, updater):
        ...
        mock_bar = MagicMock(
            timestamp=datetime.now(timezone.utc), open=150.0, high=151.0, low=149.0, close=150.5, volume=1000
        )
