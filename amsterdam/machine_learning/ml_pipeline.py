...
class MLConfig:
    ...
    # Models to train
    enabled_models: list[str] = field(default_factory=lambda: ["logistic_regression", "random_forest", "lightgbm"])

    @classmethod
    def from_file(cls, path: str | None = None) -> "MLConfig":
...
class MLPipeline:
    ...
    def _get_model_configs(self) -> dict[str, dict]:
        ...
        if "logistic_regression" in self.config.enabled_models:
            configs["Logistic_Regression"] = {
                "model": LogisticRegression(
                    class_weight="balanced", max_iter=2000, solver="saga", random_state=self.config.seed
                ),
                "params": {
        ...
        if "random_forest" in self.config.enabled_models:
            configs["Random_Forest"] = {
                "model": RandomForestClassifier(class_weight="balanced", n_jobs=-1, random_state=self.config.seed),
                "params": {
    ...
    def _create_preprocessor(self, X: pd.DataFrame) -> ColumnTransformer:
        ...
        return ColumnTransformer([("num", num_transformer, num_features), ("cat", cat_transformer, cat_features)])
    ...
    def train(
        self, data: pd.DataFrame | None = None, symbols: list[str] | None = None, tag: str = "training"
    ) -> dict[str, dict]:
        """
        ...
        for model_name, config in model_configs.items():
            ...
            pipeline = Pipeline([("preprocessor", preprocessor), ("model", config["model"])])
...
def main():
    ...
    parser.add_argument("--symbols", "-s", type=str, default="AAPL", help="Comma-separated symbols")
    parser.add_argument("--tag", "-t", type=str, default="training", help="Run tag")
