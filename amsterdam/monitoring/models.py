...
class SymbolsTableModel(QtCore.QAbstractTableModel):
    ...
    def data(self, index, role=QtCore.Qt.DisplayRole):
        ...
        if role == QtCore.Qt.BackgroundRole and c == 11:
            ...
            if risk > 60:
                return QtGui.QBrush(QtGui.QColor(60, 0, 0, 80))
            if risk > 40:
                return QtGui.QBrush(QtGui.QColor(60, 40, 0, 80))
            if risk > 20:
                return QtGui.QBrush(QtGui.QColor(0, 40, 40, 80))
    ...
    def data(self, index, role=QtCore.Qt.DisplayRole):
        ...
        if role == QtCore.Qt.ForegroundRole:
            if c == 2 and isinstance(val, str):
                if any(k in val.upper() for k in ["BUY", "COVER"]):
                    return QtGui.QBrush(QtGui.QColor("#22c55e"))
                if any(k in val.upper() for k in ["SELL", "SHORT"]):
                    return QtGui.QBrush(QtGui.QColor("#f87171"))
            return QtGui.QBrush(QtGui.QColor("#e5e5e5"))
        if role == QtCore.Qt.BackgroundRole:
            st = str(row.get("status", ""))
            if st == "REJECTED":
                return QtGui.QBrush(QtGui.QColor(80, 0, 0, 100))
            if st == "CANCELED":
                return QtGui.QBrush(QtGui.QColor(30, 30, 30, 100))
        if role == QtCore.Qt.TextAlignmentRole:
