...
def format_slack_message(data):
    ...
    if not data or 'error' in data:
        return {
            ...
            "blocks": [
                {
                    "type": "header",
                    "text": {"type": "plain_text", "text": "❌ Amsterdam Daily Summary - Error"}
                },
                {
                    "type": "section",
                    "text": {"type": "mrkdwn", "text": f"Could not generate summary: {data.get('error', 'Unknown error')}"}
                }
            ]
        }
    ...
    blocks = [
        {
            "type": "header",
            "text": {"type": "plain_text", "text": "📊 Amsterdam Daily Summary"}
        },
        {
            "type": "section",
    ...
    if data['trades']:
        ...
        blocks.append({
            ...
            "text": {
                "type": "mrkdwn",
                "text": f"*Recent Activity (last 5):*\n{trade_text}"
            }
        })
    else:
        blocks.append({
            "type": "section",
            "text": {"type": "mrkdwn", "text": "_No trades recorded today_"}
        })
    ...
    if data['errors']:
        ...
        blocks.append({
            ...
            "text": {
                "type": "mrkdwn",
                "text": f"⚠️ *Errors ({data['total_errors']}):*\n{error_text}"
            }
        })
