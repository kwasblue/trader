...
def load_state():
    ...
    if STATE_FILE.exists():
        try:
            ...
            pass
    return {"last_check": None, "alerted_today": [], "last_log_position": 0, "process_was_running": True}
...
def send_alert(webhook_url, title, message, severity="critical"):
    """Send alert to Slack"""
    emoji_map = {"critical": "🚨", "warning": "⚠️", "success": "✅"}

    color_map = {"critical": "#ef4444", "warning": "#f59e0b", "success": "#10b981"}
...
def check_process_status(state, webhook_url):
    ...
    if state["process_was_running"] and not is_running:
        ...
        send_alert(webhook_url, "Amsterdam Restarted", "✅ *Amsterdam trading bot is now running again*", "success")
        ...
        if any(keyword in line.lower() for keyword in ["connection failed", "timeout", "unreachable", "api error"]):
            if "connection_issue" not in state["alerted_today"]:
                send_alert(
                    webhook_url,
                    "Connection Issue",
                    "⚠️ *Network or API connection problem detected*\n\nAmsterdam may be unable to trade.",
                    "warning",
                )
