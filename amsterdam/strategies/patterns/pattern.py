...
class PatternDetector:
    ...
    def detect_doji(self):
        ...
        self.data["Doji"] = np.where(
            abs(self.data["Close"] - self.data["Open"]) / (self.data["High"] - self.data["Low"]) < 0.1, 1, 0
        )
        return self.data
    ...
    def detect_hanging_man(self):
        ...
        self.data["Hanging_Man"] = np.where(
            ((self.data["High"] - self.data["Low"]) > 3 * (self.data["Open"] - self.data["Close"]))
            & ((self.data["High"] - self.data["Close"]) / (0.001 + self.data["High"] - self.data["Low"]) > 0.6)
            & ((self.data["High"] - self.data["Open"]) / (0.001 + self.data["High"] - self.data["Low"]) > 0.6),
            1,
            0,
        )
        return self.data
    ...
    def detect_volume_spikes(self, threshold=1.5):
        ...
        """
        self.data["Volume_Spike"] = np.where(
            self.data["Volume"] > threshold * self.data["Volume"].rolling(window=20).mean(), 1, 0
        )
        return self.data
    ...
    def detect_volume_divergence(self):
        ...
        for i in range(1, len(self.data)):
            if (
                ...
                and self.data["Volume"][i] < self.data["Volume"][i - 1]
            ):
