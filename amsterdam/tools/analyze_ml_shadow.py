...
def match_predictions_to_outcomes(
    ...
    for trade_id, trade in trades.items():
        trades_by_symbol[trade['symbol']].append({
            'trade_id': trade_id,
            **trade
        })
...
def main():
    ...
    parser.add_argument('--shadow', default='logs/ml_filter_shadow.jsonl',
                        help='Path to shadow log file')
    parser.add_argument('--trades', default='logs/meta_trades_live.jsonl',
                        help='Path to trade outcomes file')
    parser.add_argument('--window', type=int, default=5,
                        help='Time window (minutes) for matching predictions to trades')
