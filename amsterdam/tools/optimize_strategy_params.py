...
class StrategyParamOptimizer:
    ...
    def __init__(self, symbols: list[str] | None = None, data_pipeline: UnifiedDataPipeline | None = None):
        ...
        self.logger = Logger("param_optimizer.log", "ParamOptimizer", propagate=True, level=10).get_logger()
        ...
        self.logger.info(f"StrategyParamOptimizer initialized for {len(self.symbols)} symbols")
    ...
    async def optimize_all(self, days: int = 365, metric: str = "sharpe_ratio") -> dict[str, dict[str, ParamOptResult]]:
        """
    ...
    async def _optimize_symbol(self, symbol: str, days: int, metric: str) -> dict[str, ParamOptResult]:
        """
        ...
        for regime in REGIME_TYPES:
            ...
            self.logger.info(f"  Optimizing {regime}: {strategy_name}")

            try:
                result = await self._optimize_strategy_regime(
                    symbol=symbol, regime=regime, strategy_name=strategy_name, data=data, metric=metric
                )
                ...
                self.logger.warning(f"  Failed to optimize {strategy_name} for {regime}: {e}")

        return results

    async def _optimize_strategy_regime(
        self, symbol: str, regime: str, strategy_name: str, data, metric: str
    ) -> ParamOptResult | None:
        """
        ...
        if not param_grid:
            self.logger.debug(f"    No parameter grid for {strategy_name}, using defaults")
        ...
        backtester = RegimeBacktester(
            data=data, symbol=symbol, initial_capital=100000, transaction_cost=0.001, strategies=[strategy_name]
        )
        ...
        self.logger.debug(f"    Testing {total_combos} parameter combinations...")
        ...
        for combo in product(*param_values):
            ...
            try:
                ...
                result = backtester._backtest_regime_segment(
                    strategy_name=strategy_name, regime=regime, signals=signals
                )
        ...
        self.logger.info(f"    Best params: {best_params} (Sharpe: {best_metrics.get('sharpe_ratio', 0):.2f})")
    ...
    def save_params(self, output_path: str | None = None) -> Path:
        """
...
Examples:
  ...
  python tools/optimize_strategy_params.py --days 1000 --save
    ...
    parser.add_argument("--symbols", nargs="+", help="Symbols to optimize (default: all from strategy_routing.json)")
    ...
    parser.add_argument("--save", action="store_true", help="Save to config/strategy_params.json")
    ...
    print("This will test many parameter combinations - may take 30-60 minutes\n")

    await optimizer.optimize_all(days=args.days, metric=args.metric)
