...
class AdaptiveBacktester:
    ...
    def __init__(self, symbol: str, bars: pd.DataFrame, logger=None):
        ...
        self.logger.info(
            f"AdaptiveBacktester init: has_Close={'Close' in self.bars_with_caps.columns}, has_close={'close' in self.bars_with_caps.columns}, shape={self.bars_with_caps.shape}"
        )
        ...
        try:
            with open(continuous_config_path) as f:
                ...
                self.logger.info(
                    f"Loaded continuous config: skip_threshold={self.continuous_sizing_config.get('skip_trade_threshold', 'NOT SET')}"
                )
    ...
    def _detect_regime_unstable(self, bar_idx: int) -> str:
        ...
        atr_pct = self.metrics_calc.calculate_atr_percentile(self.symbol, window)
    ...
    def _detect_regime_stable(self, detector: StableRegimeDetector, bar_idx: int) -> str:
        """
    ...
    def _calculate_position_size_fixed(self, capital: float, max_pct: float = 0.10) -> tuple[float, float]:
        """
    ...
    def _calculate_position_size_continuous(
        self, sizer: ContinuousPositionSizer, strategy: str, capital: float, max_pct: float, bar_idx: int
        ...
        recent_trades = [
            {"pnl": t.pnl, "entry_value": t.position_size, "return_pct": t.return_pct, "timestamp": t.entry_time}
    ...
    def run_backtest(
        ...
        if regime_method == "stable":
            regime_detector = StableRegimeDetector(config=regime_config or {}, logger=self.logger)
        ...
        if sizing_method == "continuous":
            ...
            position_sizer = ContinuousPositionSizer(config=final_sizing_config, logger=self.logger)
        ...
        for i in range(warmup_bars, len(self.bars)):
            ...
            if position is None and signal == "BUY":
                ...
                if sizing_method == "continuous":
                    ...
                    size, multiplier = self._calculate_position_size_fixed(capital, max_position_pct)
        ...
        # Calculate metrics
        metrics = self._calculate_metrics(equity_curve, regime_history, regime_method, sizing_method)

        return metrics

    def _calculate_metrics(
        self, equity_curve: list[float], regime_history: list[str], regime_method: str, sizing_method: str
    ) -> BacktestMetrics:
        ...
        # Regime switches
        regime_switches = sum(1 for i in range(1, len(regime_history)) if regime_history[i] != regime_history[i - 1])
...
def compare_configurations(symbol: str, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    for regime_method, sizing_method, name in configurations:
        ...
        metrics = backtester.run_backtest(regime_method=regime_method, sizing_method=sizing_method)

        result = {"Configuration": name, "Regime": regime_method, "Sizing": sizing_method, **asdict(metrics)}
