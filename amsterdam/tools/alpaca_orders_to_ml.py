...
def fetch_all_orders(paper: bool = True) -> list[Any]:
    ...
    while True:
        request = GetOrdersRequest(status=QueryOrderStatus.ALL, limit=500, nested=True, until=until)
...
def calculate_atr(symbol: str, at_time: datetime, data_client, lookback_days: int = 50) -> tuple[float, float] | None:
    """
    ...
    try:
        ...
        df = pd.DataFrame([{"high": b.high, "low": b.low, "close": b.close} for b in bar_list])
...
def calculate_time_features(timestamp: datetime) -> dict[str, Any]:
    ...
    return {"hour_of_day": hour, "day_of_week": day_of_week, "minutes_since_open": minutes_since_open}
...
def main():
    ...
    parser.add_argument(
        ...
        "-o",
        type=str,
        default="data/alpaca_trades_ml.parquet",
        help="Output file path (supports .parquet, .csv, .jsonl)",
    )
    parser.add_argument(
        "--fetch-atr", action="store_true", help="Fetch ATR from historical data (slower but more accurate)"
    )
    parser.add_argument("--stats", action="store_true", default=True, help="Print statistics")
