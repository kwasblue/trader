...
class AdaptiveSizingTuner:
    ...
    def _calculate_objective(
        self, sharpe: float, max_drawdown: float, win_rate: float, num_trades: int, params: dict
    ) -> float:
        """
    ...
    def evaluate_params(self, params: dict, train_bars: pd.DataFrame, test_bars: pd.DataFrame) -> TuningResult:
        """
        ...
        train_result = train_bt.run_backtest(
            regime_method="stable", sizing_method="continuous", initial_capital=100000, sizing_config=config
        )
    ...
    def objective_function(self, param_values: list) -> float:
        ...
        params = {dim.name: val for dim, val in zip(self.param_space, param_values)}
    ...
    def tune_bayesian(self, n_trials: int = 50) -> dict:
        ...
        self.logger.info(f"Starting Bayesian optimization: {n_trials} trials, {self.n_windows} walk-forward windows")
    ...
    def tune_random(self, n_trials: int = 50) -> dict:
        ...
        Returns:
            Best parameter dictionary
        """
        self.logger.info(f"Starting random search: {n_trials} trials, {self.n_windows} walk-forward windows")
        ...
        for trial in range(n_trials):
            ...
            self.logger.info(f"Trial {trial + 1}/{n_trials}: Objective={-objective:.3f}")

        self.logger.info("Random search complete!")
...
def main():
    ...
    for symbol in symbols:
        ...
        tuner = AdaptiveSizingTuner(
            symbol=symbol, bars=bars, train_fraction=0.7, n_windows=args.n_windows, logger=logger
        )
