        ...
        if action == 'buy':
            if positions[symbol] and positions[symbol][0]['side'] == 'short':
                ...
                positions[symbol].append({
                    'qty': qty, 'price': price, 'side': 'long',
                    'date': date, 'hour': hour, 'timestamp': ts,
            ...
            if positions[symbol] and positions[symbol][0]['side'] == 'long':
                ...
                positions[symbol].append({
                    'qty': qty, 'price': price, 'side': 'short',
                    'date': date, 'hour': hour, 'timestamp': ts,
                    'strategy': strategy, 'regime': regime,
                })
...
def print_by_day(trades_df: pd.DataFrame):
    ...
    by_day = trades_df.groupby('entry_date').agg({
        'pnl': ['count', 'sum'],
        'win': 'sum'
    }).round(2)
    ...
    for date, row in by_day.iterrows():
        ...
        print(f"{str(date):<12} {int(row['trades']):>7} {int(row['wins']):>6} "
...
def print_by_symbol(trades_df: pd.DataFrame):
    ...
    for symbol, row in by_symbol.iterrows():
        ...
        pnl_str = f"${row['pnl']:+,.2f}"
        print(f"{symbol:<8} {int(row['trades']):>7} {int(row['wins']):>6} "
...
def print_by_hour(trades_df: pd.DataFrame):
    ...
    for hour, row in by_hour.iterrows():
        ...
        print(f"{hour_str:>6} {int(row['trades']):>7} {int(row['wins']):>6} "
...
def print_by_strategy(trades_df: pd.DataFrame):
    ...
    for strat, row in by_strat.iterrows():
        pnl_str = f"${row['pnl']:+,.2f}"
        print(f"{strat:<20} {int(row['trades']):>7} {int(row['wins']):>6} "
              f"{row['win_rate']:>6.1f}% {pnl_str:>12}")
...
def print_by_hold_time(trades_df: pd.DataFrame):
    ...
    by_hold = trades_df.groupby('hold_bucket', observed=True).agg({
        'pnl': ['count', 'sum', 'mean'],
        'win': 'sum'
    }).round(2)
    ...
    for bucket, row in by_hold.iterrows():
        pnl_str = f"${row['pnl']:+,.2f}"
        avg_str = f"${row['avg_pnl']:+,.2f}"
        print(f"{str(bucket):<12} {int(row['trades']):>7} {int(row['wins']):>6} "
              f"{row['win_rate']:>6.1f}% {pnl_str:>12} {avg_str:>10}")
...
def print_worst_trades(trades_df: pd.DataFrame, n: int = 10):
    ...
    for _, row in worst.iterrows():
        ...
        print(f"{row['symbol']:<8} {row['side']:<6} ${row['entry_price']:>7.2f} "
...
def print_open_positions(positions: dict):
    ...
    for pos in open_list:
        print(f"{pos['symbol']:<8} {pos['side']:<6} {pos['qty']:>6} "
              f"${pos['price']:>10.2f} {str(pos['date']):<12}")
...
def main():
    ...
    parser.add_argument(
        ...
        help=f"Path to trades CSV (default: {DEFAULT_LOG})"
    )
