...
class AdaptiveTrader:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        self.continuous_sizer = ContinuousPositionSizer(
            config=self.config.get('position_sizing', {}),
            logger=self.logger
        )
    ...
    def get_strategy_for_regime(self, symbol: str, regime: str) -> Dict:
        ...
        return {
            'strategy': regime_config.get('strategy', 'sma'),
            'timeframe': regime_config.get('timeframe', 'day')
        }
    ...
    def execute_trade_decision(self, symbol: str, base_capital: float = 100000):
        ...
        self.logger.info(f"4. Position Sizing:")
        ...
        if shares == 0:
            self.logger.warning(f"5. SKIP TRADE - Position too small (${position_info['size']:.0f} / ${current_price:.2f} = 0 shares)")
            return None
