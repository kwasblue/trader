...
class MockExecutor(BaseExecutor):
    ...
    def __init__(self, broker: BaseBrokerInterface, event_handler: EventHandler | None = None):
        """
        ...
        self.logger = Logger(log_file="mock_executor.log", logger_name="MockExecutor", propagate=True).get_logger()
    ...
    def place_oco_order(self, symbol: str, qty: int, stop_price: float, limit_price: float) -> bool:
        ...
        """
        self.logger.warning(f"[{symbol}] OCO orders not implemented in MockExecutor")
        return False
    ...
    def _place_order(self, symbol: str, qty: int, side: OrderSide, price: float) -> bool:
        """
        ...
        try:
            self.logger.info(f"[MOCK] Placing {side.value} order: {symbol} {qty}@${price:.2f}")

            # Place order via broker (instant fill)
            result = self.broker.place_market_order(symbol=symbol, qty=qty, side=side, price=price)

            if not result.success:
                self.logger.error(f"[{symbol}] Order failed: {result.message}")
                self._emit_alert("error", f"Order failed for {symbol}: {result.message}", symbol)
                return False

            self.logger.info(f"[MOCK] Order filled: {side.value} {qty}@${price:.2f}")
            ...
            self._emit_alert("error", f"Order failed for {symbol}: {str(e)}", symbol)
            return False
    ...
    def _emit_alert(self, level: str, message: str, symbol: str | None = None) -> None:
        """Emit alert event."""
