...
class BaseStrategy(ABC):
    ...
    @abstractmethod
    def generate_signal(self, data: pd.DataFrame | dict[str, Any]) -> int:
        """
    ...
    @trace
    def generate_signal_traced(self, data: pd.DataFrame | dict[str, Any]) -> int:
        """
    ...
    def validate_data(self, data: pd.DataFrame | dict[str, Any]) -> bool:
        """
        ...
        if isinstance(data, pd.DataFrame):
            ...
            raise ValueError(f"Invalid data type: {type(data)}. Expected pd.DataFrame or dict")
